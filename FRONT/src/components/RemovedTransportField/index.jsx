...
import { Button, Form, Modal, Table, Tooltip } from 'components'
import { AuthContext } from 'contexts'
import dayjs from 'dayjs'
import React, { useCallback, useContext, useEffect, useState } from 'react'
import COLORS from 'constants/colors'

function RemovedTransportField({form, description='', onSelect}) {
  ...
  useEffect(() => {
    if(selectedDate && state.userProfileData){
      ...
      axios({
        ...
        // setFlights(res.data.filter((item) => item.Direction === data.ReScheduleDirection))
    ...
    <>
      <Tooltip title='Removed schedule'>
        <Form.Item className='mb-0 w-full'>
          <Input disabled status='error' prefix={<CloseCircleOutlined/>} value={description} className='w-full'/>
        </Form.Item>
      </Tooltip>
...
export default RemovedTransportField