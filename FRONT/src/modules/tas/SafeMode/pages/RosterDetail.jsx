...
import { Alert, Form as AntForm, InputNumber, Select, message, notification } from 'antd'
...
function RosterDetail({data}) {
  ...
  const errorShow = (err) => {
    try {
      JSON.parse(err.response?.data?.message.data);
      setWarningModal(true)
      setErrorMessage(JSON.parse(err.response?.data?.message))
    } catch (error) {
  ...
  const customCellRender = (e, data) => {
    ...
    let offSiteStatus = data.EmployeeOffSiteStatus?.find((item) => dayjs(item.EventDate).format(dateFormat) === dayjs(e).format(dateFormat))
