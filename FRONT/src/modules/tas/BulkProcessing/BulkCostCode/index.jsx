...
function BulkCostCode() {
  ...
  const handleDownload = () => {
    ...
    axios({
      ...
      responseType: 'blob',
      data: {
        costCodeIds: selectedRowKeys
      }
    }).then(async (res) => {
      const url = await window.URL.createObjectURL(new Blob([res.data]));
      const fn = `TAS_CostCode_Bulk_Download_${dayjs().format('YYYY-MM-DD hh:mm:ss')}.xlsx`
  ...
  const handleUpload = (file) => {
    if(validateFile(file)){
      ...
      formData.append('BulkCostCodeFile', file)
      if(file){
        axios({
          method: 'post',
          url: 'tas/costcode/bulkuploadpreview',
          headers: {
  ...
  const handleUploadEmployees = (file) => {
    if(validateFile(file)){
      ...
      if(file){
        axios({
          method: 'post',
          url: 'tas/costcode/bulkuploademployeespreview',
          headers: {
            'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
          },
          data: formData,
  ...
  const handleSubmitUpload = () => {
    setUploadLoading(true)
    const formData = new FormData();
    formData.append('BulkCostCodeFile', uploadFile)
    if(uploadFile){
      axios({
        method: 'post',
        url: 'tas/costcode/bulkupload',
        headers: {
          'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        },
        data: formData,
  ...
  const handleSubmitUploadEmployees = () => {
    setUploadLoading(true)
    const formData = new FormData();
    formData.append('BulkCostCodeFile', uploadEmpFile)
    if(uploadEmpFile){
      axios({
        method: 'post',
  ...
  const handleDownloadRow = () => {
    ...
    axios({
      method: 'post',
      url: 'tas/costcode/bulkrequestemployees',
      responseType: 'blob',
      data: {
        costCodeId: selectedExportRow.Id
      }
    }).then(async (res) => {
      const url = await window.URL.createObjectURL(new Blob([res.data]));
  ...
  return (
    <div>
      <div className='rounded-ot bg-white px-3 py-2 mb-3 shadow-md'>
        <div className='text-lg font-bold mb-2'>Bulk Cost Code</div>
        <Form
          form={form}
          fields={fields}
      ...
      <Table
        ...
        autoExpandAll={true}
        loading={loading}
        tableClass='max-h-[calc(100vh-250px)]'
