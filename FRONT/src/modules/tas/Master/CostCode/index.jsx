...
const fields = [
  ...
  },
  {
    ...
    name: 'Number',
    className: 'col-span-12 mb-2',
    rules: [{required: true, message: 'Number is required'}, { min: 5, message: 'Number must be at least 5 character'}],
    inputprops: {
      maxLength: 100
    } 
  },
  {
    ...
    name: 'Description',
    className: 'col-span-12 mb-2',
    rules: [{required: true, message: 'Description is required'}, { min: 5, message: 'Number must be at least 5 character'}], 
  },
]
...
function CostCode() {
  ...
  const getReferDataCostCode = () => {
    action.changeLoadingStatusReferItem({ costCodes: true })
    axios({
      method: 'get',
      url: 'tas/costcode?Active=1',
    }).then((res) => {
      let tmp = []
    ...
    }).then(() => action.changeLoadingStatusReferItem({ costCodes: false }))
  }
