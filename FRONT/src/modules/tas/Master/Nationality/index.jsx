import { ActionForm, Form, Table, Button, Modal, AuditTable } from 'components'
...
function Nationality() {
  ...
  useEffect(() => {
    action.changeMenuKey('/tas/nationality')
    getData()
  },[])
  ...
  const getReferDataNationality = () => {
    action.changeLoadingStatusReferItem({ nationalities: true })
    axios({
      method: 'get',
      url: 'tas/nationality?Active=1',
    }).then((res) => {
      let tmp = []
    ...
    }).then(() => action.changeLoadingStatusReferItem({ nationalities: false }))
  }
