...
function PeopleType() {
  ...
  useEffect(() => {
    action.changeMenuKey('/tas/peopletype')
    getData()
  },[])
  ...
  const getReferDataPeopleType = () => {
    action.changeLoadingStatusReferItem({ peopleTypes: true })
    axios({
      method: 'get',
      url: 'tas/peopletype?Active=1',
    }).then((res) => {
      let tmp = []
    ...
    }).then(() => action.changeLoadingStatusReferItem({ peopleTypes: false }))
  }
  ...
  return (
    <div>
      ...
      <AuditTable
        title='People Type Audit' 
        tablename={'PeopleType'} 
        open={showAudit} 
