...
import { Button, Form, Modal, PeopleSearch, Table } from 'components'
...
function Supervisors({data, getData, departmentData}) {
  ...
  const handleSubmitAdd = (employeeData) => {
    setLoading(true)
    axios({
      method: 'post',
      url: 'tas/department/supervisor',
      data: {
        departmentSupervisorId: employeeData.Id,
  ...
  const handleDelete = () => {
    ...
    axios({
      method: 'delete',
      url: `tas/department/supervisor/${editData.Id}`,
    }).then(() => {
      getData()
      setShowPopup(false)
    }).catch(() => {
  ...
  const setPrimary = () => {
    ...
    axios({
      method: 'put',
      url: `tas/department/setmainsupervisor?Id=${editData.Id}`,
    }).then(() => {
      getData()
  ...
  return (
    <div className='max-w-[800px]'>
      ...
      <Drawer title='Add Supervisor' open={showModal} width={1000} onClose={() => setShowModal(false)}>
        <PeopleSearch
      ...
      <Popup
        ...
        width={350}
      >
        <div className='text-center'>Are you sure you want to assign <span className='font-bold'>{editData?.FullName}</span> the main supervisor?</div>
        <div className='flex gap-5 mt-4 justify-center'>
