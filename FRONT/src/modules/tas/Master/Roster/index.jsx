...
function Roster() {
  ...
  const getReferDataRoster = () => {
    action.changeLoadingStatusReferItem({ rosters: true })
    axios({
      method: 'get',
      url: 'tas/roster?Active=1',
    }).then((res) => {
      let tmp = []
    ...
    }).then(() => action.changeLoadingStatusReferItem({ rosters: false }))
  }
  ...
  const fields = [
    ...
    },
    {
      label: 'Roster Group',
      name: 'RosterGroupId',
      rules: [{required: true, message: 'Roster Group is required'}],
      className: 'col-span-12 mb-2',
      type: 'select',
      inputprops: {
        options: rosterGroup
  ...
  return (
    <div>
      ...
      <Table
        ...
        keyExpr='Id'
        pager={false}
        onRowDblClick={(e) => navigate(`/tas/roster/${e.data.Id}`)}
