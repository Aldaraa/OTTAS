...
function RequestHotel() {
  ...
  useEffect(() => {
    action.changeMenuKey('/tas/requesthotel')
    getData()
  },[])
  ...
  const columns = [
    ...
    },
    {
      ...
      name: 'DayCost',
      cellRender: ({value}) =>(
        <div>{Intl.NumberFormat().format(value)}</div>
      )
    },
    {
      label: 'Early Check In Cost',
      name: 'EarlyCheckInCost',
      cellRender: ({value}) =>(
        <div>{Intl.NumberFormat().format(value)}</div>
      )
    },
    {
      label: 'Late Check Out Cost',
