...
import { Form as AntForm, Checkbox, DatePicker, TreeSelect } from 'antd';
import { useLocation } from 'react-router-dom';
import { AuthContext } from 'contexts';
import dayjs from 'dayjs';
import columns from './columns';

function ChangesDepartment() {
  ...
  const handleSubmit = (values) => {
    ...
    axios({
      method: 'put',
      url: `tas/employee/changedata/department`,
      data: tmp,
    }).then((res) => {
  ...
  return (
    <div>
      ...
      <Modal 
        ...
        title={<div>Change Department <span className='text-gray-400 font-normal'>({selectedData.length} people selected)</span></div>} 
        width={1000}
        forceRender={true}
      >
        <AntForm 
                      ...
                      <tr className={`${(name+1)%2 === 0 ? 'bg-[#fafafa]' : 'white'}`}>
                        ...
                        <td className='py-1 border-t px-1'>
                          <AntForm.Item
                            ...
                            className='mb-0'
                          >
                            <div className='text-[13px]'>{form.getFieldValue(['rosters', name, 'DepartmentName'])}</div>
                          </AntForm.Item>
                        </td>
                        <td className='py-1 border-t px-1'>
                          <AntForm.Item
...
export default ChangesDepartment