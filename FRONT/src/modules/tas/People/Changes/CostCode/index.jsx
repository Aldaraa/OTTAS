...
function ChangesEmployer() {
  ...
  const handleSubmit = (values) => {
    ...
    axios({
      method: 'put',
      url: `tas/employee/changedata/costcode`,
      data: tmp,
    }).then((res) => {
  ...
  return (
    <div>
      ...
      <Modal 
        ...
        title={<div>Change Cost Code <span className='text-gray-400 font-normal'>({employees?.length} people selected)</span></div>} 
        width={1100}
        forceRender={true}
      >
        <AntForm 
                ...
                <tbody>
                  {
                    fields.map(({key, name, ...restField}) => (
                      <tr className={`${(name+1)%2 === 0 ? 'bg-[#fafafa]' : 'white'}`} key={key}>
                        ...
                        <td className='py-1 border-t px-1'>
                          <AntForm.Item
                            ...
                            className='mb-0'
                          >
                            <div className='text-[13px]'>{form.getFieldValue(['rosters', name, 'CostCodeName'])}</div>
                          </AntForm.Item>
                        </td>
                        <td className='py-1 border-t px-1'>
                          <AntForm.Item
