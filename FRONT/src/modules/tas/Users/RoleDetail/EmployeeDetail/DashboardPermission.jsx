...
function DashboardPermission({employeeData, refreshData}) {
  ...
  const getReports = () => {
    axios({
      method: 'get',
      url: `tas/sysroleemployeedashboard/getdashboard/${employeeData?.EmployeeId}`
    }).then((res) => {
      let tmp = []
      res.data.map((item) => {
        if(item.Permission){
  ...
  const handleSavePermission = () => {
    ...
    selectedReportKeys.map((id) => {
      tmp.push({dashboardId: id, permission: 1})
    })
    axios({
      method: 'post',
      url: 'tas/sysroleemployeedashboard',
      data: {
        employeeId: employeeData?.EmployeeId,
        dashboardPermissions: [...tmp],
      }
    }).then((res) => {
      getReports()
      refreshData()
