...
function Camp() {
  ...
  const getData = () => {
    ...
    axios({
      ...
      action.setReferDataItem({camps: tmp})
  ...
  const getReferDataCamp = () => {
    action.changeLoadingStatusReferItem({ camps: true })
    axios({
      method: 'get',
    ...
    }).then(() => action.changeLoadingStatusReferItem({ camps: false }))
  }
    ...
    },
    {
      ...
      cellRender: (e) => (
        <div className='flex gap-4'>
          <button className='edit-button' onClick={() => handleEditButton(e.data)}>Edit</button>
          {
            e.data.Active === 1 ?
            e.data.RoomCount > 0 ? 
            <div>
  ...
  return (
    <div>
      ...
      <Table
        data={renderData}
        columns={columns}
        allowColumnReordering={false}xw
        loading={loading}
        keyExpr='Id'
