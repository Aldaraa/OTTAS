...
const title = 'Accommodation Rules'

function AccommodationConfig() {
  ...
  const getData = () => {
    ...
    axios({
      method: 'get',
      url: `tas/requestnonsiteaccommodationgroupconfig`
    }).then((res) => {
  ...
  const handleSaveReorder = () => {
    ...
    axios({
      method: 'put',
      url: 'tas/requestnonsiteaccommodationgroupconfig/order',
      data: {
        Ids: Ids,
  ...
  return (
    <div>
      ...
      />
      <Modal 
        title={`${editData ? 'Edit' : 'Add'} Group (Accommodation Rule)`}
        open={showModal} 
        onCancel={closeModal}
      >
        <Form
