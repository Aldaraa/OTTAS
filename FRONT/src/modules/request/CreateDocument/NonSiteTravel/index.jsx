...
import ProfileForm from 'modules/request/Documents/SiteTravelOTLLC/ProfileForm'
...
function CreateNonSiteTravel() {
    ...
    if(profileData?.Active === 0){
      profileForm.validateFields().then(() => {
        if(flightData.length > 0 || accommodationData.length > 0){
          ...
          axios({
            method: 'get',
            url: `tas/requestdocument/checkduplicate/non site travel/${empId}${firstFlight ? `?startdate=${dayjs(firstFlight.travelDate).format('YYYY-MM-DD')}` : ''}`
          }).then((res) => {
            ...
            if(res.data.length === 0){
              axios({
                ...
                data: {
                  ...
                  accommodationData: accommodationData ? accommodationData.map((item) => ({...item, FirstNight: dayjs(item.FirstNight).format('YYYY-MM-DD HH:mm'), LastNight: dayjs(item.LastNight).format('YYYY-MM-DD HH:mm')})) : [],
                  files: files ? files : [],
                  requestInfo: {
  ...
  return (
    <div className='px-6 py-5 rounded-ot shadow-card bg-white'>
      <Button
        className='mb-3' 
        onClick={() => navigate('/request/nonsitetravel')} 
        icon={<LeftOutlined />}
      >
        Back
      </Button>
        ...
        <div className='flex flex-col gap-4 mt-4'>
          <Form 
            ...
            initValues={{
              ...
              DocumentRoute: [],
              ...profileData, PassportExpiry: profileData?.PassportExpiry ? dayjs(profileData?.PassportExpiry) : null
            }}
            // editData={{...profileData, PassportExpiry: profileData?.PassportExpiry ? dayjs(profileData?.PassportExpiry) : null}}
            // labelWrap
            onFinish={handleSubmit}
          >
            {
              profileData?.Active === 0 ?
              <Accordion className='col-span-12 border rounded-ot overflow-hidden mt-3' title='Profile'>
                <div className='p-4 flex flex-col gap-5' ref={profileFormRef}>
                  <ProfileForm
                    data={profileData} 
                    form={profileForm}
              ...
              <div className='grid grid-cols-12 gap-y-0 gap-x-4 p-4'>
                ...
                >
                  <Select 
                    ...
                    // onChange={(e, f) => {
                    //   console.log('ee', e, f, profileData);
                    //   form.setFieldValue()
                    // }}
                  />
                </Form.Item>
              ...
              <div className='flex flex-col gap-2 p-4'>
                <div>
                  ...
                  <Form.Item className='m-0' name='comment'>
                    <Input.TextArea maxLength={300} showCount autoSize={{minRows: 3, maxRows: 5}}/>
                  </Form.Item>
                </div>
      ...
      <div className='flex justify-end items-center mt-4 gap-4'>
        ...
        {/* <Button onClick={handleDraft} disabled={actionLoading}>Save</Button> */}
