...
function Temporary({form, disabled}) {
  ...
  return (
    <Form form={form} className='max-w-[600px] p-4' size='medium' disabled={disabled}>
      ...
      <Form.Item name={'StartDate'} label='Start Date' className='col-span-12 mb-2' rules={[{required: true, message: 'Start Date is required'}]}>
        <DatePicker/>
      </Form.Item>
      <Form.Item name={'EndDate'} label='End Date' className='col-span-12 mb-2' rules={[{required: true, message: 'End Date is required'}]}>
        <DatePicker/>
      </Form.Item>
    </Form>
  )
