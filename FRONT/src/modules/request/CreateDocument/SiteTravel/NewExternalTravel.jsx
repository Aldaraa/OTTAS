...
function NewExternalTravel({onCancel, refreshData, employeeId, externalForm}) {
  ...
  const form = externalForm || newform
  ...
  const fields = [
    {
      type: 'component',
      component: <>
      <Form.Item label={<div><span className='text-red-400 text-xs'>*</span> First Transport</div>} className='col-span-12 mb-2'>
        <div className='flex gap-2'>
          ...
          <Form.Item className='mb-0 w-[90px]'>
            <Input disabled value={'EXTERNAL'} className='w-full'/>
          </Form.Item>
          <Form.Item noStyle name={['firstTransport', 'flightId']}>
          </Form.Item>
          <Form.Item name={['firstTransport', 'Description']} className='flex-1 mb-0' rules={[{required: true, message: 'First Transport Schedule is required'}]}>
            <Input readOnly/>
          </Form.Item>
          <Form.Item
