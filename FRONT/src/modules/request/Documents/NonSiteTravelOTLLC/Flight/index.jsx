...
function Flight({disabled, documentDetail, currentGroup, getData, data, ref, hasActionPermission, userInfo}) {
  ...
  // const onCopyWithHeaders = () => {
  //   const datagrid = dataGrid.current?.instance;
  //   const selectedRowsData = datagrid.getSelectedRowsData();
  //   const columnHeaders = datagrid.getVisibleColumns().map(col => col.caption);
  //   const rowsHeaders = datagrid.getVisibleRows().map((row) => row.values)
  //   console.log('grid', datagrid.getVisibleRows());

  //   const csvContent = [
  //     columnHeaders.join('\t'), // headers
  //     ...rowsHeaders.map(row => row.join('\t'))
  //   ].join('\n');
    
  //   navigator.clipboard.writeText(csvContent).then(() => {
  //     alert('Copied to clipboard with headers!');
  //   }, (err) => {
  ...
  const columns = [
    ...
    },
    {
      label: '',
      name: 'action',
      width: `${disabled ? '0px' : '220px'}`,
      cellRender: (e) => (
        !disabled &&
        <div className='flex gap-4'>
          {/* <button type='button' className='edit-button' onClick={() => onCopyWithHeaders()}>Copy</button> */}
  ...
  const isEditable = useMemo(() => {
    ...
    if(state.userInfo.Role === 'TravelAdmin' || (currentGroup?.GroupTag === "requester" && documentDetail?.AssignedEmployeeId === userInfo?.EmployeeId)){
      returnStatus = true
    }
    return returnStatus
  ...
  const isDeletable = useMemo(() => {
    return state.userInfo.Role === 'TravelAdmin'
  },[state.userInfo])
  ...
  return (
    <div className='col-span-1' id='flight' ref={ref}>
      <Tabs 
        type='card'
        activeKey={tabKey}
