...
function ProfileForm({data, form, documentData, className, refreshData, disabled, getProfile}) {
  ...
  const groupFields = [
    ...
    },
    {
      label: 'OTUP Grouping',
      name: 'otup',
      className: 'col-span-6 lg:col-span-6 mb-0',
      type: 'select',
      inputprops: {
        options: []
      }
    ...
    if(documentData?.DocumentTag === 'temp'){
      ...
      Object.keys(values).map((item) => {
        ...
        if(item !== 'PassportRawImage' && item !== 'SiteContactEmployeeId'){
          ...
          if(item === 'CompletionDate' || item === 'CommenceDate' || item === 'Dob'){
  ...
  const items = [
    ...
    {
      label: <div>Group {renderErrors(groupFields)}</div>,
      key: 3,
      forceRender: true,
