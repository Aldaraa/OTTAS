...
namespace Persistence.Repositories
{
    ...
    {
        public async Task<string> TransportDetailsQuerySMS(List<ReportCol> FieldNames, string reportName, List<ReportParam> reportParams)
        {
            ...
            var query = @$"SELECT t.EventDate, Profile.Department, Profile.Employer, t.EmployeeId PersonId, CONCAT(Profile.Firstname, ' ', Profile.Lastname) Person,  Profile.SAP SAPID, t.Direction, tm.Code TransportMode,
                            at.code TransportCode,
                            ts.Description Description, Profile.NRN, Profile.PersonalMobile            {fields}  from Transport t 
                           LEFT JOIN Employee ProfileData ON t.EmployeeId = ProfileData.Id                          
INNER JOIN GetReportProfileData({employeeId}) Profile on t.EmployeeId = Profile.PersonNo
                          ...
                          WHERE
                         t.EventDate >= '[STARTDATE]' and t.EventDate <= '[ENDDATE]' and t.Direction IN ('IN', 'OUT') 
                            [WHERECONDITION]
                         ORDER BY t.EventDate,  CONCAT(Profile.Firstname, ' ', Profile.Lastname) asc;";
            ...
            return await SaveToTransportDetailsQuerySMSExcelFile(query, @"\Assets\GeneratedFiles\", "TAS Transport Details SMS", reportParams);
        ...
        private async Task<string> SaveToTransportDetailsQuerySMSExcelFile(string queryData, string filePath, string reportName, List<ReportParam> reportParams)
        {
